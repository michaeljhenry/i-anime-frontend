...
const EditAnimePage = () => {
  ...
  useEffect(() => {
    const getAnime = async () => {
      const animeData = await sendRequest(
        process.env.REACT_APP_BACKEND_URL + `/animes/${aid}`,
        "GET",
        null,
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      await sendRequest(
        process.env.REACT_APP_BACKEND_URL + `/animes/patch/${aid}`,
        "PATCH",
    ...
    <React.Fragment>
      ...
      {!isLoading && (
        <div className="anime-form__container">
          ...
          {animeInfo.type !== "toWatch" && (
            <div className="anime-form__score" onChange={scoreChangeHandler}>
              <p>Anime Score: </p>
              <select defaultValue={animeInfo.score} name="scores" id="scores">
          ...
          <button
            disabled={
              animeInfo.type === "watched" && !(score || animeInfo.score)
            }
